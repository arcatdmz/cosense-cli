...
- 通常の節見出しには原則として `[* 見出し]` を使う。`[** 見出し]` 以上は、ページ全体を大きく区切る必要がある場合だけに限定する
- 年月日を特定できる日付は原則として日付ページへのリンク `[YYYY-MM-DD]` にする。曜日付きは `[YYYY-MM-DD] (曜)`、時刻付きは `[YYYY-MM-DD] (曜) HH:mm` とし、各要素の間に半角スペースを入れる
- 年が確定できない日付や、内容を忠実に残すべき引用原文は無理に日付リンクへ変換しない
