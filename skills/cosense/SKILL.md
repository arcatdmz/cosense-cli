...
- 箇条書き文化に合わせて、簡潔な一行単位の記述を基本にする
- 一行一文、または一行一情報を基本にする
- 通常の Scrapbox/Cosense 行末には句点 `。` を付けない
- 一行に複数文が入りそうな場合は行を分ける
- 短すぎて分けるとかえって読みにくい場合は `、` でつなぐ
- 見出しやイベントの下に詳細を置く場合はインデントで整理する
- この文体は Scrapbox/Cosense 書き込みや下書きに適用し、報告書・メール・論文・公式文書の文体までは縛らない
